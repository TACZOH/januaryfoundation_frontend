...
export class Form {
	...
	public async submit(
		...
		return await axios
			.post(`${config.ENDPOINT_URL}${API(this.type)}`, requestData)
			.then(async (result) => {
	...
	public Phone(
		number: string,
		config?: { country?: string; validateMobilePrefix?: boolean; strictDetection?: boolean }
	): {
		valid: boolean;
		number: string;
	} {
			...
			business: (data: Data): IBusinessApplication => {
				return {
					Business: {
						...
						tellUsAboutYourBusiness: data.Organization.tellUsAboutYourBusiness
					},
					PhysicalLocation: {
...
function Phone(
	...
	const number: string =
		typeof PhoneResult.phoneNumber === 'string' ? PhoneResult.phoneNumber : phoneNumber;
	return {
