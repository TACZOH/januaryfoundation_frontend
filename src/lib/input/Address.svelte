<script lang="ts">
	...
	import { countries, states } from "$lib/stores";

	function renderStates(fromCountry: string) {
		// phoneCode.set(country.findByName(fromCountry)?.dialing_code ?? '');
		$states = countryList.findByName(fromCountry)?.provinces ?? [];
	}
...
{#if $states[0] !== undefined}
	<div class="sm:col-span-2">
		<Listbox
			...
			let:open>
			<ListboxLabel class="block text-sm font-medium text-gray-700">State / Province</ListboxLabel>
			<div class="mt-1">
				<ListboxButton
					class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
					...
					<ListboxOptions
						...
						{#each $states as state}
							<ListboxOption
								...
								class={({ active }) =>
									active
										? "active text-gray-900 cursor-default select-none relative py-2 pl-8 pr-4"
										: "text-gray-900 cursor-default select-none relative py-2 pl-8 pr-4"}
								let:selected>
								{#if selected}
...
<!-- <div class="sm:col-span-2">
	...
	<div class="mt-1">
		<input
			...
			autocomplete="on"
			class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md"
			placeholder={country.findByName($selectedCountry)?.capital ?? 'City'}
		/>
	</div>
</div> -->
...
<div class="sm:col-span-6">
	<label for="additional-address" class="block text-sm font-medium text-gray-500">
		Additional address (optional)
	</label>
	<div class="mt-1">
		<input
			bind:value={value.optional}
			type="text"
