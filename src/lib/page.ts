import type { IHomePage, IFaqPage, IBlogPage, IRafflePage } from '$lib/types';
...
export const page: PromisePage = {
	home: async (): Promise<IHomePage> => {
		return await axios
			.get(`${config.ENDPOINT_URL}home-page`)
			.then(async (response) => {
	...
	faq: {
		application: async (): Promise<IFaqPage> => {
			return await axios
				.get(`${config.ENDPOINT_URL}faq-page`)
				.then(async (response) => {
					return await response.data;
				})
				.catch((error) => {
		...
		raffle: async (): Promise<IFaqPage> => {
			return await axios
				.get(`${config.ENDPOINT_URL}raffle-faq-page`)
				.then(async (response) => {
					return await response.data;
				})
				.catch((error) => {
	...
	blog: async (): Promise<IBlogPage> => {
		return await axios
			.get(`${config.ENDPOINT_URL}blog-page`)
			.then(async (response) => {
				return await response.data;
			})
			.catch((error) => {
