<script context="module" lang="ts">
	import type { IFaq, IHomePage, IFaqPage } from '$lib/types';
	...
	export const load: Load = async ({ fetch }) => {
		const res = await fetch(`${config.ENDPOINT_URL}faqs`);
		const faqs = await res.json();
		const page: IFaqPage = await pages.faq.application();
...
<div class='bg-white overflow-hidden shadow divide-y divide-gray-200'>
	...
	<section class="max-w-7xl mx-auto pb-32">
		<div class="mb-20 text-center">
			<h2 class="mt-8 md:text-5xl font-bold text-3xl">Grant</h2>
		</div>
